...
# %% auto 0
__all__ = ['API_URL', 'semaphore', 'search', 'update_batch', 'bulk_update_async', 'update']
        ...
        async with semaphore:
            async with session.post(API_URL, json={"companies": payload}, headers=headers) as response:
                if response.status == 429:
                    ...
                    retry_count += 1
                    #tqdm.write(f"⚠️ Rate limited. Retrying in {retry_after} sec...")
                ...
                # if retry_count: tqdm.write(f"✅ Batch succeeded after {retry_count} retries.")
                return True
